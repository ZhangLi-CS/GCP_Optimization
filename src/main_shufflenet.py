...
parser.add_argument('--decay_step', default=10000, type=int, metavar='N',
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            #utils.load_state_ckpt(args.resume,model)
        else:
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        adjust_learning_rate(optimizer,i, epoch, args)
        if args.gpu is not None:
...
def adjust_learning_rate(optimizer,iters, epoch, args):
    iter_step = epoch * args.decay_step + iters
    lr = args.lr*(1-iter_step/(args.decay_step*args.epochs))
